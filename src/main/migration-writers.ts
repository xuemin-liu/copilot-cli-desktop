...
export async function assertMigrationWritersStopped(deps = {
  ...
  if (daemon && deps.processAlive(daemon.pid)) throw new Error('The saved background controller process is still running, but its status could not be verified. Stop it or retry when it responds.')
