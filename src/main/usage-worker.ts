...
async function collect(backup: 'daily' | 'forced' | 'none' = 'daily'): Promise<UsageFlushResult> {
  ...
  if (backup !== 'none') return value.tryBackup(backup === 'forced')
  return { backupWarning: value.backupWarning(), backupDiagnostic: null }
}
  ...
  queue = queue.then(async () => {
    ...
    try {
      ...
      switch (message.method) {
        ...
        case 'restore': {
          const restored = recovered ? null : ledger!.restoreFrom(message.args[0] as string)
          const collected = await collect()
          // Follow-up collection can skip a throttled backup. Keep this restore's
          // detail only while its warning is still current, without caching it.
          result = { ...collected, backupDiagnostic: collected.backupDiagnostic ??
            (restored && restored.backupWarning === collected.backupWarning ? restored.backupDiagnostic : null) }
          break
        }
